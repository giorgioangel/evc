...
sim_number <- as.integer(args[1])
traj_number <- as.integer(args[2])
trial <- as.integer(args[3])
# data output (platform independent construction)
folder_input <- file.path('chain')
# riverswim problem specification
state.count <- 64                 # number of states in the riverswim problem
...
domains_paths <- paste("flake64", as.character(sim_number), sep="/")
if(!dir.exists(domains_paths)) dir.create(domains_paths, recursive = TRUE)
...
domain_spec <- load_domain(domains_paths)
...
wd <- getwd()
if(!dir.exists(domains_paths)) dir.create(domains_paths, recursive = TRUE)
file_path <- paste(domains_paths, "/traj/traj_", as.character(traj_number), "_", as.character(trial), ".csv", sep="")
...
folder_output <- file.path(domains_paths, 'posteriors', paste(as.character(traj_number), as.character(trial), sep="_"))
