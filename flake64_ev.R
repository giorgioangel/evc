...
# whether to print partial results duchain the computation
print_partial <- FALSE
...
p <- paste("flake64/", as.character(sim_number), "/posteriors/", as.character(traj_number), "_", as.character(trial), sep="")
...
load_domain <- function(dir_path){
    ...
    if(file.size(training_file) < 10e6){
        ...
        if(!file.exists(training_raw)) {
            cat("            Large, decompressing using xz ... \n")
            system2("xz", paste("-d -k", training_file))
        }
    ...
    if(file.size(test_file) < 10e6){
        ...
        if(!file.exists(test_raw)) {
            cat("            Large, decompressing using xz ... \n")
            system2("xz", paste("-d -k", test_file))
        }
